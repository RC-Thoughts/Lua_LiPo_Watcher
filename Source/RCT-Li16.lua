--[[
    ...
    Requires DC/DS-14/16 with firmware 4.22 or up.
    ---------------------------------------------------------
    LiPo Watcher is part of RC-Thoughts Jeti Tools.
    ---------------------------------------------------------
            Released under MIT-license by 
        Tero Salminen @ RC-Thoughts.com 2017-2019
    ---------------------------------------------------------
--]]
collectgarbage()
----------------------------------------------------------------------
...
local timeNow,timeLast,timeFill,voltMax,rX=0,0,0,0
----------------------------------------------------------------------
-- Read translations
local function setLanguage()
    local lng=system.getLocale()
    local file=io.readall("Apps/Lang/RCT-LiP2.jsn")
...
----------------------------------------------------------------------
-- Telemetry for Small-display
local function dispLiPoSmall(percentage,ox,oy)
    -- Fuel bar 
    lcd.drawRectangle (71+ox,4+oy,16,15)
    lcd.drawRectangle (54+ox,4+oy,16,15)  
    lcd.drawRectangle (37+ox,4+oy,16,15)  
    lcd.drawRectangle (20+ox,4+oy,16,15)  
    lcd.drawRectangle (3+ox,4+oy,16,15)
    -- Bar chart
    if(cellPerc >= 0) then
        local nSolidBar=math.floor(cellPerc / 20)
        local nFracBar=(cellPerc-nSolidBar * 20) / 20
        ...
        -- Solid bars
        for i=0,nSolidBar-1,1 do 
            lcd.drawFilledRectangle (4+i*17+ox,5+oy,14,13) 
        end  
        -- Fractional bar
        local x=math.floor(4+nSolidBar*17)
        lcd.drawFilledRectangle (x+ox,5+oy,15*nFracBar,13)
    end
end
----------------------------------------------------------------------
-- Telemetry for Clock-display
local function dispLiPoClock(percentage,ox,oy)
    -- Fuel bar 
    lcd.drawRectangle (59+ox,3+oy,13,13)
    lcd.drawRectangle (45+ox,3+oy,13,13)  
    lcd.drawRectangle (31+ox,3+oy,13,13)  
    lcd.drawRectangle (17+ox,3+oy,13,13)  
    lcd.drawRectangle (3+ox,3+oy,13,13)
    -- Bar chart
    if(cellPerc >= 0) then
        local nSolidBar=math.floor(cellPerc / 20)
        local nFracBar=(cellPerc-nSolidBar * 20) / 20
        ...
        -- Solid bars
        for i=0,nSolidBar-1,1 do 
            lcd.drawFilledRectangle (4+i*14+ox,4+oy,11,11) 
        end  
        -- Fractional bar
        local x=math.floor(4+nSolidBar*14)
        lcd.drawFilledRectangle (x+ox,4+oy,11*nFracBar,11)
    end
end
----------------------------------------------------------------------
...
local function dispLiPo(width,height)
    if(height==69)then -- Big window
        ...
        elseif(width==75 and height==30) then -- Clock Display
                if (cellPerc==-1) then
                    lcd.drawText(40-lcd.getTextWidth(FONT_MINI,"-%"),12,"-%",FONT_BOLD)
                else
                    lcd.drawText(40-lcd.getTextWidth(FONT_BOLD,string.format("%s%%",cellPerc)),12,string.format("%s%%",cellPerc),FONT_BOLD)
                end
                lcd.drawText(72-lcd.getTextWidth(FONT_MINI,string.format("%.2fV",cellVolt)),16,string.format("%.2fV",cellVolt),FONT_MINI)
                dispLiPoClock(cellPerc,1,0)
    else -- Small window
        if (cellPerc==-1) then
...
local function loop()
    local sensor = {}
    local sensorRx = system.getTxTelemetry()
    if(sensId == 999) then
        if(sensPa == 1) then
            sensor.value = sensorRx.rx1Voltage
            elseif (sensPa == 2) then
            sensor.value = sensorRx.rx2Voltage
            elseif (sensPa == 3) then
            sensor.value = sensorRx.rxBVoltage
        end
        if(sensorRx.rx1Percent > 1)then
            sensor.valid = true
        else
            sensor.valid = false
        end
